    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stok_masuk  = $_POST['stok']; // stok yang ditambahkan
        $stok_lama   = $data['stok'];  // stok yang sudah ada
        $stok_total  = $stok_lama + $stok_masuk;
        $harga       = $_POST['harga'];
        $modal       = $_POST['modal'];
...
class="bg-white p-6 rounded-2xl shadow max-w-xl space-y-4">
    ...
    <div class="bg-gray-100 p-3 rounded-lg">
        ...
        <p class="text-xl font-bold text-teal-600"><?= $data['stok'] ?> Buku</p>
    </div>

    <!-- TAMBAH STOK -->
    <div>
        <label class="text-sm font-semibold">Tambah Stok</label>
        <input type="number" name="stok" value="0" min="0"
        class="w-full border rounded-lg px-4 py-3"
        placeholder="Masukkan jumlah stok yang ingin ditambahkan">

        <p class="text-xs text-gray-500 mt-1">
            Isi jika ingin menambah stok. Kosongkan atau isi 0 jika tidak menambah stok.
        </p>
    </div>
